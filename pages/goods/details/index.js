...
onShareAppMessage() {
  ...
  async addCart({ detail: { count, pickedSku } }) {
    ...
    const records = await fetchCartItems(_id);
    const cartItem = records.find((x) => x.sku._id === pickedSku._id);
    // eslint-disable-next-line eqeqeq
    if (cartItem == null) {
      // cart item empty, create
      if (count <= pickedSku.count) {
        console.log('rl',records);
        await createCartItem({ skuId: pickedSku._id, count,uid:_id }).then(addCartSucceed, addCartFail);
      } else {
        overCount();
      }
    } else {
